...
resource "null_resource" "name" {
  ...
  provisioner "file" {
    source      = "fs_docker.sh"
    destination = "/home/ec2-user/fs_docker.sh"
  }

  # set permissions and run the build_docker_image.sh file
  provisioner "remote-exec" {
    inline = [
      "sudo chmod +X /home/ec2-user/fs_docker.sh",
      "sh /home/ec2-user/fs_docker.sh",
    ]
  } 
